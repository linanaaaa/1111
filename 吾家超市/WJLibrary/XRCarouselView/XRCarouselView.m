...
- (void)setScrollViewContentSize {
    if (_images.count > 1) {
        ...
        self.currImageView.frame = CGRectMake(self.scrollView.frame.size.width * 2, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
        
        if (_changeMode == ChangeModeFade) {
            //淡入淡出模式，两个imageView都在同一位置，改变透明度就可以了
            _currImageView.frame = CGRectMake(0, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
...
//        [self startTimer];
        ...
        self.currImageView.frame = CGRectMake(0, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
        [self stopTimer];
    }
}
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    CGFloat centerY = self.scrollView.frame.size.height - size.height * 0.5 - VERMARGIN - (_describeLabel.hidden?0: DES_LABEL_H);
    CGFloat pointY = self.scrollView.frame.size.height - size.height - VERMARGIN - (_describeLabel.hidden?0: DES_LABEL_H);
    
    if (_pagePosition == PositionDefault || _pagePosition == PositionBottomCenter)
        _pageControl.center = CGPointMake(self.scrollView.frame.size.width * 0.5, centerY);
    else if (_pagePosition == PositionTopCenter)
        _pageControl.center = CGPointMake(self.scrollView.frame.size.width * 0.5, size.height * 0.5 + VERMARGIN);
    else if (_pagePosition == PositionBottomLeft)
        _pageControl.frame = CGRectMake(HORMARGIN, pointY, size.width, size.height);
    else
        _pageControl.frame = CGRectMake(self.scrollView.frame.size.width - HORMARGIN - size.width, pointY, size.width, size.height);
}
...
- (void)nextPage {
    ...
    } else [self.scrollView setContentOffset:CGPointMake(self.scrollView.frame.size.width * 3, 0) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    _describeLabel.frame = CGRectMake(0, self.scrollView.frame.size.height - DES_LABEL_H, self.scrollView.frame.size.width, DES_LABEL_H);
    //重新计算pageControl的位置
    self.pagePosition = self.pagePosition;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX < self.scrollView.frame.size.width * 2) {
        if (_changeMode == ChangeModeFade) {
            self.currImageView.alpha = offsetX / self.scrollView.frame.size.width - 1;
            self.otherImageView.alpha = 2 - offsetX / self.scrollView.frame.size.width;
        } else self.otherImageView.frame = CGRectMake(self.scrollView.frame.size.width, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
        
        self.nextIndex = self.currIndex - 1;
        if (self.nextIndex < 0) self.nextIndex = _images.count - 1;
        self.otherImageView.image = self.images[self.nextIndex];
        if (offsetX <= self.scrollView.frame.size.width) [self changeToNext];
        
    //向左滚动
    } else if (offsetX > self.scrollView.frame.size.width * 2){
        if (_changeMode == ChangeModeFade) {
            self.otherImageView.alpha = offsetX / self.scrollView.frame.size.width - 2;
            self.currImageView.alpha = 3 - offsetX / self.scrollView.frame.size.width;
        } else self.otherImageView.frame = CGRectMake(CGRectGetMaxX(_currImageView.frame), 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
        
        self.nextIndex = (self.currIndex + 1) % _images.count;
        self.otherImageView.image = self.images[self.nextIndex];
        if (offsetX >= self.scrollView.frame.size.width * 3) [self changeToNext];
    }
}
